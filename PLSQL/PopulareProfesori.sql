...
create table Profesori(
  ...
  CONSTRAINT fk_Profesori_id_liceu FOREIGN KEY (id_liceu) REFERENCES Liceu(id),
  CONSTRAINT fk_Materii_id_materie FOREIGN KEY (id_materie) REFERENCES Materii(id)
)
/
...
declare
  ...
  cursor liceu is select * from Liceu;
...
begin
  ...
  for v_indexLiceu in liceu loop
    v_init2 := v_indexLiceu.id;
    
    for v_indexMaterii in materii loop
      v_init1 := v_indexMaterii.id;
      v_init3 := v_init3 + 1;
      
      v_varsta := ceil(dbms_random.value(25,65));
      ...
      v_email := v_email  || v_init3 || sfarsitMail(ceil(dbms_random.value(0,c_mail)));
    ...
    insert into PROFESORI values (v_init3,v_nume,v_prenume,v_gen,v_varsta,v_adresa,v_telefon,v_email,v_init2,v_init1);
    insert into CONTURI values (v_username,v_password,v_init3);
