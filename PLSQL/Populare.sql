...
create table Parinti(
  ...
  constraint fk_Parinti_id_elev foreign key (id_elev) references Elevi(id)
)
/
...
begin 
    ...
    for v_index in elevi loop
      v_adresa := v_index.adresa;
      v_indexL := 2 * v_index.id - 1;
      v_indexE := v_index.id;
      v_varsta := ceil(dbms_random.value(35,50));
      v_nume := v_index.nume;
      ...
      v_telefon := v_telefon || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) 
       ...
       v_email := v_email  || v_indexL || sfarsitMail(ceil(dbms_random.value(0,c_mail)));
    ...
    insert into PARINTI values (v_indexL,v_nume,v_prenume,v_gen,v_varsta,v_adresa,v_telefon,v_email,v_indexE);
    insert into CONTURI values (v_username,v_password,v_indexL);
      
      v_telefon := '0';
      v_prenume := '';
      v_prenume1 := '';
      v_gen := '';
      v_username := 'Pa.' || lower(v_nume);
      v_password := '';
      v_email := '';  
      v_indexL := 2 * v_index.id;
      v_email := lower(v_nume);
      v_telefon := v_telefon || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) 
                             || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9)) || ceil(dbms_random.value(0,9));
